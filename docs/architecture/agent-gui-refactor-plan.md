...
状态：讨论稿，用于架构对齐会。范围：agent 活动数据的模型协议、`packages/agent/activity-core`、`packages/agent/gui`，以及它们的全部消费方。不在范围内：daemon 内部各 provider 适配器的实现细节（这部分是健康的，见 2.8 节）。
...
先说健康的部分：运行时适配层不需要重构。`standard_acp_adapter.go` 是共享引擎，每个走 ACP 协议的 provider 只是一份 86 到 160 行的声明式配置（如 `acp_provider_opencode.go` 仅 86 行）。需要修的是适配层之外的两处。
...
| OpenCode                    | 标准 ACP         | 声明式配置 86 行                   |
| Codex                       | 专属协议         | 已迁出 ACP，走代码生成             |
| Claude Code                 | SDK 边车（默认） | ACP 桥只是环境变量可切回的遗留路径 |
| Tutti Agent                 | 专属协议家族     |                                    |
| nexight / hermes / openclaw | 标准 ACP         |                                    |

Codex 和 Claude Code 都是从 ACP 迁出的。这不是历史偶然，而是能力驱动的必然趋势：ACP 是最小公分母协议，provider 需要更深的能力（评审、目标模式、后台任务、上下文压缩、结构化补丁）就会迁向原生协议。架构结论：传输协议不是稳定边界，归一化的活动契约才是；必须假设每个 provider 最终都可能拥有专属运行时（Claude 从 ACP 切到 SDK 时的那批对齐修复就是迁移成本的实证），因此能力必须由 daemon 下发，而不是界面按 provider 名字猜。
...
provider 切片（依赖 P1 的能力描述符 schema）：🚧 通用描述符类型与消费适配层已经落地。✅ Codex 已完成单 provider 收口：runtime/status/installer/auth watcher、model/skill/capability/slash catalog、turn lifecycle 投影、系统 target、事件归一化均由 descriptor 驱动；slash policy 通过 OpenAPI 显式契约下发，GUI 的 plan/权限/skill/slash 行为只读能力、目录与 invocation，不再按 Codex 名称猜测；identity/target/icon key 生成物及漂移检查已接入，检查同时覆盖全语言 locale key 与完整图标资源，未知图标不再静默回退。其余 provider 的迁移、注册表与全量枚举最终相等校验，以及未迁移 provider 的 legacy fallback 清理尚未完成。
