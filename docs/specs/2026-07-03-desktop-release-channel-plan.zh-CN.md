...
需要把 “stable / rc” 拆成三层来识别，避免把 GitHub Latest、electron-updater
channel 和 S3 `latest.json` 混成一个概念。
...
所以 GitHub 侧的 stable / rc 识别，核心不是看 workflow 名字，而是看最终解析出来的
release tag 是否带 `-rc.N`。
...
- `allowPrerelease=false` 时：
  ...
  - 只要 GitHub Release 的 `make_latest` 没有被 RC 覆盖，stable 用户不会收到 RC。
- `allowPrerelease=true` 时：
  - 会读取 GitHub releases Atom feed。
  - 如果显式设置了 `autoUpdater.channel = "rc"`，它会按 tag 的 semver prerelease
    第一段筛选，即只接受 `*-rc.*`。
  - 它不会把自定义 `rc` 当成官方稳定层级里的 `alpha` / `beta`，所以不会自动让
    `rc` 用户升级到 stable。
  - 找到 RC tag 后，会优先读取该 tag 下的 `rc-mac.yml` / `rc.yml`。
...
- rc 侧不能只说 “设置 channel=rc 就万事大吉”，还要保证 RC release 资产里有正确的
  update yaml。
- 如果暂时只有 `latest-mac.yml`，RC 更新仍可能因为 fallback 可用，但这不是清晰契约。
- 更稳妥的做法是发布 RC 时显式生成 / 上传 `rc-mac.yml`。dry-run 验证发现当前
  electron-builder macOS 产物只生成 `latest-mac.yml`，所以 workflow 需要在 RC 构建后
  把它物化为 `rc-mac.yml`，并继续上传 `apps/desktop/dist/*.yml`。
...
- RC 可以上传到不可变 tag 目录，但不能覆盖根目录 `latest.json`。
...
pre-release / RC 包可以上传到 S3 的不可变 tag 目录，供内部 QA 通过固定链接下载，
...
```json
{
  ...
  "summary": {
    ...
    "en": {
      ...
      "qaFocus": ["Verify fresh macOS install, upgrade from an older build, and external download links."],
      "risks": ["Download links may briefly point to the previous version while CDN caches expire."]
    }
  },
  "markdown": {
