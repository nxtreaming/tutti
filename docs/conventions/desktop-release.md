...
The mirrored desktop release also writes mutable `latest.json` metadata at the release asset prefix root. That file lists the current desktop release tag, version, stable `tutti.sh/desktop/download` URLs, and CloudFront/static URLs for every uploaded asset:
