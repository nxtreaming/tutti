...
describe("useAgentGUINodeController", () => {
  ...
  it("moves the empty home composer to the first ready provider while startup detection continues", async () => {
    ...
    expect(nextData.agentTargetId).toBe("local:claude-code");
    expect(result.current.viewModel.providerReadinessGate).toBeNull();
  });
  ...
  it("selects target-backed rail targets for the empty composer", async () => {
    ...
    // updater asserted below and reflected immediately by the selected target.
  ...
  describe("active session agent-target fail-fast", () => {
    ...
    it("fails fast for a legacy provider-only session without an agent target id", async () => {
      const { result } = renderFailFast({
        sessions: [
  ...
  it("does not hydrate target-backed composer settings from stale provider defaults", async () => {
    const getComposerOptions = vi.fn(async () => ({
      provider: "claude-code",
      modelConfig: {
        configurable: true,
    ...
    expect(
      result.current.viewModel.composerSettings.selectedModelValue
    ).not.toBe("gpt-5.5");
    expect(
      result.current.viewModel.composerSettings.selectedPermissionModeValue
    ).not.toBe("auto");
  ...
  it("passes only the selected agent target id when starting a new conversation", async () => {
  ...
  it("blocks an internal target without an explicit agentTargetId", async () => {
    const activate = vi.fn(
      async (input: AgentHostActivateAgentSessionInput) => ({
        session: agentSession(input.agentSessionId, {
          provider: input.mode === "new" ? input.provider : "codex"
  ...
  it("does not synthesize provider targets when the host directory is empty", () => {
    installAgentHostApi({
      list: vi.fn(async () => ({ presences: [], sessions: [] })),
      listSessionTimeline: vi.fn(async () => ({ timelineItems: [] })),
      subscribeEvents: vi.fn(() => vi.fn())
  ...
  it.each([["codex"], ["claude-code"]] as const)(
    "enters the first created conversation immediately before activation resolves (provider: %s)",
    async (provider) => {
      let resolveActivate:
      ...
      act(() => {
        result.current.actions.submitPrompt(
          promptBlocks("start the first turn")
        );
      });
      ...
      // resolves, and no per-session loading flag is left on. This holds
      // identically for every provider: the entry timing lives entirely in
      // the shared controller, with no provider-specific branching.
      expect(
        getAgentSessionView({
          workspaceId: "room-1",
          agentSessionId: capturedAgentSessionId
  ...
  it("keeps an unavailable OpenClaw agent disabled even when the gateway is ready", async () => {
        ...
        agentTargets: [
          {
            ...
            agentTargetId: "local:openclaw",
            provider: "openclaw",
            ref: { kind: "agent-directory", provider: "openclaw" },
            label: "OpenClaw",
            disabled: true
          }
        ],
  ...
  it("keeps home visible and preserves draft when activation fails immediately", async () => {
    ...
    // The optimistic entry is reverted and the failure is surfaced on the
    // home composer instead of being silently swallowed.
    expect(result.current.viewModel.detailError).toBe("runtime not connected");
    expect(toast.error).not.toHaveBeenCalled();
  });
  ...
  it("clears the per-session messages-loading flag when a new conversation activation fails", async () => {
    ...
    // The failed create must not leave global or per-session loading state
    // behind, but it does surface the error on the home composer it reverted
    // to.
    expect(result.current.viewModel.detailError).toBe("runtime not connected");
    expect(result.current.viewModel.isLoadingMessages).toBe(false);
    expect(
  ...
  it("updates the active session and remembers the switched fields as target defaults", async () => {
    ...
    const updatedData = onDataChange.mock.calls
      ...
      .at(-1);
    expect(
      updatedData?.composerOverridesByAgentTargetId?.["local:codex"]
    ).toMatchObject({
      model: "gpt-5.1",
      reasoningEffort: "high"
    });
  ...
  it("records node defaults at switch time even when the active session settings update fails", async () => {
    ...
    const updatedData = onDataChange.mock.calls
      ...
      .at(-1);
    expect(
      updatedData?.composerOverridesByAgentTargetId?.["local:claude-code"]
    ).toMatchObject({
      model: "MiniMax-M2.7",
      reasoningEffort: "medium"
    });
  ...
  it("reloads new session ACP options after the selected agent target changes", async () => {
    const getComposerOptions = vi.fn(async () => ({
      provider: "codex",
      effectiveSettings: {},
      runtimeContext: {
    ...
    await waitFor(() => {
      expect(getComposerOptions).toHaveBeenCalledWith(
        expect.objectContaining({ agentTargetId: "target-a" })
      );
    });
    ...
    await waitFor(() => {
      expect(getComposerOptions).toHaveBeenCalledWith(
        expect.objectContaining({ agentTargetId: "target-b" })
      );
    });
  });
...
function agentGuiData(
  ...
  const agentTargetId = Object.prototype.hasOwnProperty.call(
    overrides,
    "agentTargetId"
  )
    ? overrides.agentTargetId
    : `local:${provider}`;
  const composerOverridesByAgentTargetId =
    overrides.composerOverridesByAgentTargetId ??
    (agentTargetId && overrides.composerOverrides
      ? { [agentTargetId]: overrides.composerOverrides }
      : undefined);
  return {
    provider,
